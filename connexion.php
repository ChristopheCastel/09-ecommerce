<?php
    ...
    // Si l'indice 'action' est définit dans l'URL et qu'il a pour valeur 'deconnexion', cela veut dire que l'internaute a cliqué sur le lien 'deconnexion' et donc transmise dans l'URL les paramètres 'action=deconnexion', alors on entre dans la condition IF et on supprime l'indice 'user' dans la session afin qu'il ne soit plus authentifié sur le site

    if(isset($_GET['action']) && $_GET['action'] == 'deconnexion')
    {
        // echo "Je veux me déconnecter";
        unset($_SESSION['user']);
    }
    ...
    if(isset($_POST['pseudo_email'], $_POST['password'], $_POST['submit']))
    {
        ...
        {   
            ...
            if(password_verify($_POST['password'], $user['password']))
            {
                ...
                // On crée un tableau multidimensionnel dans la session, ici on crée un indice 'user' dans lea session qui a pour valeur un tableau ARRAY contenant toutes les données de l'internaute authentifié sur le site
