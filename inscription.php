<?php
    ...
    // Si l'indice 'user' est défini dans la session (connect()), cela veut dire que l'internaute est authentifié sur le site, il n'a rien à faire sur la page inscription, on le redirige (header()) vers la page profil.php 
    if(connect())
    {
    ...
    /*
        Exercice :
        1. Contrôler en PHP que l'on réceptionne bien toute les données saisie dans le formulaire (print_r)
        ...
        5. Faites en sorte d'informer l'internaute si le email n'est pas du bon format (filter_var)
    ...
    */
    ...
    {
        // $pdo->exec("INSERT INTO membre (pseudo, password, nom, prenom, email, civilite, ville, code_postal, adresse) VALUES('$_POST[pseudo]' '$_POST[password]', '$_POST[nom]', '$_POST[prenom]', '$_POST[email]', '$_POST[civilite]', '$_POST[ville]', '$_POST[code_postal]', '$_POST[adresse]')");
        ...
        //2. Faites en sorte d'informer l'internaute si le champ 'pseudo' est laissé vide
            ...
            {
                $erreurPseudo = '<small class="fst-italic text-danger">Votre champ est vide.</small>';
                $error = true;
            }
        ...
        //3. Faites en sorte d'informer l'internaute si le pseudo n'est pas disponible (SELECT + ROWCOUNT)
            // On sélcetionne tout dans la table 'membre' à condition que la colonne 'pseudo' soit égal au 'pseudo' saisi dans le formulaire
            ...
            //echo 'il y a ' . $verifPseudo->rowCount() . ' pseudo dèjà utilisé<hr>';
            ...
            {
                $erreurPseudo = "<small class='fst-italic text-danger'>Nom d'utilisateur indisponible, merci d'en saisir un nouveau</small>";
                $error = true;
            }
        ...
        //4. Faites en sorte d'informer l'internaute si le champ 'email' est laissé vide
            ...
            echo 'il y a ' . $verifEmail->rowCount() . ' email dèjà utilisé<hr>';

            if(empty($_POST['email']))
            {
                $errorEmail = '<small class="fst-italic text-danger">Votre champ est vide.</small>';
                $error = true;               
            }
        ...
        //7. Faites en sorte d'informer l'internaute si le champ 'password' ou 'confirm_password' sont laissé vide
            ...
            {
                $errorPassword = "<small class='fst-italic text-danger'>Votre champ est vide</small>";
                $error = true;
            }
            elseif($_POST['password'] != $_POST['confirm_password'])
            {
                //8. Faites en sorte d'informer l'internaute si les mots de passe ne correspondent pas 
                $errorPassword = "<small class='fst-italic text-danger'>Les mots de passe ne correspondent pas</small>";
                $error = true;
            }


            // Si l'indice name 'pdc' n'est pas définit, cela veut dire que l'internaute n'a pas coché la case des politiiques de confidentialités
            if(!isset($_POST['pdc']))
            {
        ...
        // if(!isset($error))
        // {
        //     // Requête SQL d'insertion (INSERT + PREPARE + BINDVALUE + EXECUTE)
            
        //      $bdd->exec("INSERT INTO membre (civilite, pseudo, password, email, nom, prenom,adresse, ville, code_postal) VALUES ('$_POST[civilite]', '$_POST[pseudo]', '$_POST[password]', '$_POST[email]', '$_POST[nom]', '$_POST[prenom]', '$_POST[adresse]', '$_POST[ville]', '$_POST[code_postal]')");
        // }
        ...
        if(!isset($error))
        {
            ...
            $insertMembre = $bdd->prepare("INSERT INTO membre (civilite, pseudo, password, email, nom, prenom, adresse, ville, code_postal) VALUES (:civilite, :pseudo, :password, :email, :nom, :prenom, :adresse, :ville, :code_postal)");
            ...
            $_SESSION['valid_inscription'] = "<p class='col-7 bg-success text-white text-center mx-auto p-3 mt-3'>Félicitation ! Vous êtes maintenant inscrit sur le site. Vous pouvez dès à présent vous connecter.</p>"; 


            // On redirige l'intenaute vers la page connexion.php après execution de la requête d'insertion, après l'inscription de l'internaute
    ...
    // $goodJob = "<small class='fst-italic text-success'>Bravo ! Vous êtes inscrit</small>";
            // echo "<p class='bg-success text-black text-center p-3 col-6 mx-auto>Félicitations !! Vous êtes maintenant inscrit sur le site !</p>";



            // $newUser = $bdd->prepare("INSERT INTO membre (civilite, pseudo, password, confirm_password, email, nom, prenom,adresse, ville, code_postal) = :moreUser");
            // $newUser->bindValue(':moreUser', $_POST['civilite'], $_POST['pseudo'], $_POST['password'], $_POST['confirm_password'], $_POST['email'], $_POST['nom'], $_POST['prenom'], $_POST['adresse'], $_POST['ville'], $_POST['code_postal'] ,PDO::PARAM_STR);
            // $newUser->execute(); 
            ...
            // $newUser = $bdd->prepare("INSERT INTO membre (civilite, pseudo, password, confirm_password, email, nom, prenom,adresse, ville, code_postal) VALUES ('$_POST[civilite], $_POST[pseudo], $_POST[password], $_POST[confirm_password], $_POST[email], $_POST[nom], $_POST[prenom], $_POST[adresse], $_POST[ville], $_POST[code_postal]') = :email");
            // $newUSEr->bindValue(':email', $_POST['email'],PDO::PARAM_STR);
            // $verifEmail->execute();
...
?>
            ...
            <form method="post" class="row g-3 mb-5">
                ...
                <div class="col-md-6">
                    ...
                    <input type="text" class="form-control <?php if(isset($erreurPseudo)) echo $border; ?>" id="pseudo" name="pseudo" value="<?php if(isset($_POST['pseudo'])) echo $_POST['pseudo']; ?>">
                ...
                <div class="col-md-6">
                    ...
                    <input type="password" class="form-control <?php if(isset($errorPassword) || isset($errorPassword)) echo $border; ?>" id="password" name="password">
                ...
                <div class="col-md-6">
                    ...
                    <input type="password" class="form-control <?php if(isset($errorPassword) || isset($errorPassword)) echo $border; ?>" id="confirm_password" name="confirm_password">
                </div>
                <div class="col-12">
